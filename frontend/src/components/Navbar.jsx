...
export default function Navbar() {
  ...
  const navLinks = [
    ...
    { name: t('nav.electrical_lighting'), path: '/electrical-lighting' },
    { name: t('nav.experience'), path: '/experience' },
    { name: t('nav.contact'), path: '/contact' },
    { name: t('nav.admin'), path: isAdmin ? '/admin/dashboard' : '/admin/login' }
  ];

  return (
    <nav className="sticky top-0 z-50 bg-white/80 backdrop-blur-md border-b border-slate-100 shadow-sm transition-smooth">
        ...
        <div className="flex items-center justify-between h-20">
          ...
          <div className="flex-shrink-0 flex items-center">
            <Link to="/" className="flex flex-col group">
              <span className="font-outfit font-extrabold text-xl tracking-tight text-forest-dark sm:text-2xl leading-none transition-smooth group-hover:text-forest">
                RAJA RAJESHWARI
              </span>
              <span className="font-sans font-semibold text-xs text-wood tracking-widest mt-1 transition-smooth group-hover:text-wood-dark">
                INTERIOR WORKS
              </span>
            </Link>
          </div>
          ...
          <form onSubmit={handleSearchSubmit} className="hidden md:flex relative max-w-xs w-full mx-4">
            <input
              ...
              className="w-full bg-slate-100 text-slate-800 text-sm px-4 py-2 pl-10 rounded-full border border-transparent focus:outline-none focus:bg-white focus:border-wood/30 focus:shadow-md transition-smooth"
            />
            ...
            {navLinks.map((link, index) => {
              ...
              return link.path.startsWith('/#') ? (
                <a
                  ...
                  className="font-sans font-medium text-slate-600 hover:text-forest transition-smooth text-sm relative pb-1 after:absolute after:bottom-0 after:left-0 after:h-[2px] after:w-0 hover:after:w-full after:bg-forest after:transition-all after:duration-300"
                >
                  {link.name}
                </a>
              ) : (
                <Link
                  key={index}
                  to={link.path}
                  className={`font-sans text-sm transition-smooth relative pb-1 after:absolute after:bottom-0 after:left-0 after:h-[2px] after:transition-all after:duration-300 ${
                    isActive 
      ...
      {/* Mobile Drawer Menu */}
      {mobileMenuOpen && (
        <div className="lg:hidden border-t border-slate-100 bg-white px-4 pt-2 pb-6 space-y-4 shadow-inner">
